        ...
        <div className="glass-strong rounded-2xl p-4 mb-6 animate-slide-in-bottom overflow-x-auto">
          {loading ? (
            ...
            <Table>
              ...
              <TableBody>
                {rows.map((r, i) => (
                  <TableRow key={r.id ?? `${r.player_name}-${i}`} className="border-border/30 hover:bg-primary/5">
                    <TableCell className="py-3">{rankIcon(i)}</TableCell>
                    <TableCell className="font-semibold text-foreground">{r.player_name}</TableCell>
                    <TableCell className="text-center font-bold text-foreground">
                      {r.score}/{r.total_questions}
                    </TableCell>
                    <TableCell className="text-primary">{r.category}</TableCell>
